...
public:
    ~OnScopeExit();
    void unlink(const std::string& str);
...
mve::Bundle::Ptr load_bundle_from (const std::string &scene_path);
...
TEST(SceneTest, TheInitialViewsOfACreatedSceneMatchWithThatSceneOnDisk)
{
    ...
    mve::Scene::Ptr scene_without_views = [&clean_up]()
    {
        ...
        return mve::Scene::create(path);
    }();
    EXPECT_EQ(scene_without_views->get_views().size(), std::size_t(0));

    mve::Scene::Ptr scene_with_views = [&clean_up]()
    {
        std::string path = create_scene_on_disk(73, make_bundle(23), clean_up);
        return mve::Scene::create(path);
    }();
...
TEST(SceneTest, TheInitialBundleOfACreatedSceneMatchesWithThatSceneOnDisk)
{
    ...
    {
        std::string path = create_scene_on_disk(0, make_bundle(0), clean_up);
        return mve::Scene::create(path);
    }();
    ...
    {
        std::string path = create_scene_on_disk(3, make_bundle(23), clean_up);
        return mve::Scene::create(path);
    }();
    EXPECT_TRUE(bundle_cameras_match(
...
TEST(SceneTest, CreateSceneThrowsAnExceptionIfTheViewsSubdirectoryDoesNotExist)
{
    ...
    std::string dir_with_no_views_subdir = [&clean_up]()
    {
...
TEST(SceneTest, WhenLoadIsCalledOnASceneItsPathUpdatesAccordingly)
{
    ...
    {
        std::string path = create_scene_on_disk(13, make_bundle(3), clean_up);
        return mve::Scene::create(path);
    }();
...
TEST(SceneTest, WhenLoadIsCalledOnASceneItsViewsUpdateAccordingly)
{
    ...
    std::string loaded_path = create_scene_on_disk(9, make_bundle(4), clean_up);
    scene->load_scene(loaded_path);
    Views views_from_disk = load_views_from(loaded_path);
...
TEST(SceneTest, WhenLoadIsCalledOnASceneItsBundleUpdatesAccordingly)
{
    ...
    mve::Scene::Ptr scene = [&clean_up]()
    {
        std::string path = create_scene_on_disk(13, make_bundle(0), clean_up);
        return mve::Scene::create(path);
    }();
    std::string loaded_path = create_scene_on_disk(0, make_bundle(5), clean_up);
    scene->load_scene(loaded_path);
    mve::Bundle::Ptr bundle_from_disk = load_bundle_from(loaded_path);
...
TEST(SceneTest, WhenSaveIsCalledOnASceneTheSceneOnDiskUpdatesAccordingly)
{
    ...
    {
        std::string path = create_scene_on_disk(13, nullptr, clean_up);
...
TEST(SceneTest, WhenSaveViewsIsCalledOnASceneOnlyTheViewsAreUpdatedOnDisk)
{
    ...
    mve::Bundle::Ptr loaded_bundle = load_bundle_from(dirty_scene->get_path());
    ViewList loaded_views = load_views_from(dirty_scene->get_path());
    EXPECT_FALSE(bundle_cameras_match(loaded_bundle, dirty_scene->get_bundle()));
    EXPECT_TRUE(views_match(loaded_views, dirty_scene->get_views()));
}
...
TEST(SceneTest, ACleanSceneBecomesDirtyIfAnyOfItsViewsBecomeDirty)
{
    ...
    {
        std::string path = create_scene_on_disk(10, make_bundle(8), clean_up);
        return mve::Scene::create(path);
    }();
...
TEST(SceneTest, ADirtySceneRemainsDirtyWhenMoreOfItsElementsBecomeDirty)
{
    ...
    mve::Scene::Ptr dirty_scene = [&clean_up]()
    {
        std::string path = create_scene_on_disk(7, make_bundle(3), clean_up);
        mve::Scene::Ptr scene = mve::Scene::create(path);
        make_a_clean_view_dirty(scene);
        return scene;
    }();
...
TEST(SceneTest, SavingADirtySceneCleansIt)
{
    ...
    mve::Scene::Ptr dirty_scene = [&clean_up]()
    {
        std::string path = create_scene_on_disk(10, make_bundle(1), clean_up);
        mve::Scene::Ptr scene = mve::Scene::create(path);
        make_a_clean_view_dirty(scene);
        scene->set_bundle(make_bundle(0));
        return scene;
...
TEST(SceneTest, SaveViewsCleansASceneIfOnlyItsViewsAreDirty)
{
    ...
    {
        std::string path = create_scene_on_disk(4, make_bundle(4), clean_up);
        mve::Scene::Ptr scene = mve::Scene::create(path);
...
TEST(SceneTest, SaveViewsDoesNotCleanASceneIfItsBundleIsDirty)
{
    ...
    mve::Scene::Ptr scene_with_dirty_bundle = [&clean_up]()
    {
        std::string path = create_scene_on_disk(5, make_bundle(7), clean_up);
        mve::Scene::Ptr scene = mve::Scene::create(path);
...
TEST(SceneTest, SaveBundleCleansASceneIfOnlyItsBundleIsDirty)
{
    ...
    {
        std::string path = create_scene_on_disk(10, make_bundle(3), clean_up);
        mve::Scene::Ptr scene = mve::Scene::create(path);
...
TEST(SceneTest, SaveBundleDoesNotCleanASceneIfAnyOfItsViewsAreDirty)
{
    ...
    {
        std::string path = create_scene_on_disk(7, make_bundle(8), clean_up);
        mve::Scene::Ptr scene = mve::Scene::create(path);
        make_a_clean_view_dirty(scene);
        scene->set_bundle(mve::Bundle::create());
        return scene;
...
TEST(SceneTest, SavingTheDirtyViewsOfASceneCleansTheSceneIfItsBundleIsClean)
{
    ...
    mve::Scene::Ptr scene_with_dirty_views_and_clean_bundle = [&clean_up]()
    {
        std::string path = create_scene_on_disk(10, make_bundle(6), clean_up);
        mve::Scene::Ptr scene = mve::Scene::create(path);
        for(int i = 0; i < 5; ++i)
            make_a_clean_view_dirty(scene);
        return scene;
...
create_scene_on_disk (const std::size_t view_count,
    const mve::Bundle::Ptr bundle, OnScopeExit& on_scope_exit)
{
...
mve::Scene::ViewList
load_views_from (const std::string &scene_directory)
{
...
mve::Bundle::Ptr
load_bundle_from (const std::string &scene_directory)
{
...
views_match (mve::Scene::ViewList& lhs, mve::Scene::ViewList& rhs)
{
    ...
    for (mve::View::Ptr& left_view : lhs)
    {
        mve::Scene::ViewList::iterator found = std::find_if(
            rhs.begin(), rhs.end(), [&left_view](mve::View::Ptr& right_view) {
...
bundle_cameras_match (mve::Bundle::Ptr lhs, mve::Bundle::ConstPtr rhs)
{
    ...
    const auto match_n =
        [&match](const float l[], const float r[], std::size_t count)
    {
...
void
OnScopeExit::unlink (const std::string& str)
{
    this->paths.push_back(str);
}

void
OnScopeExit::unlink_recursive (const std::string& path)
{
    namespace fs = util::fs;
